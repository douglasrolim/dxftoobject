...
const PromiseProps = require('../util').PromiseProps
        ...
        return new Promise((resolve)=>{
            this._setEntitiesCommon(entity, 'polyline')
                .then((r)=>{
                    ...
                    let linearArr = [];
                    return PromiseEach(entity.vertices, (vertices)=>{
            ...
            if(this.dxfData.blocks.hasOwnProperty(entity.name) && (!this.dxfData.blocks[entity.name].hasOwnProperty('xrefPath') || this.dxfData.blocks[entity.name]['xrefPath'] === '' )){
                this._setEntitiesCommon(entity, 'insert')
                    .then((r)=>{
                        ...
                        insert.entities = [];
                        return PromiseEach(this.dxfData.blocks[entity.name].entities, (entity)=>{
                            return this._parseEntity(entity)
                                .then((r)=>{
        ...
        return new Promise((resolve)=>{
            this._setEntitiesCommon(entity, 'dimension')
                .then((r)=>{
                    ...
                    dimension.entities = [];
                    return PromiseEach(this.dxfData.blocks[entity.block].entities, (entity)=>{
                        return this._parseEntity(entity)
                            .then((r)=>{
        ...
        return new Promise((resolve)=>{
            this._setEntitiesCommon(entity, 'solid')
                .then((r)=>{
                    let solid = r;
                    let points = [];
                    PromiseEach(entity.points, (vertices)=>{
                ...
                if(typeof ltypes[key] === 'string' || !ltypes[key].hasOwnProperty('pattern')) return false;
                    ...
                    .then((total)=>{
                        if(total > 0) return false;
                            ...
                            let dashArray = [];
                            return PromiseEach(ltypes[key]['pattern'],(patternVal)=>{
        ...
        return new Promise((resolve)=>{
            ...
            if(typeof ltypes !== 'undefined' && ltypes.length > 0){
                if(lineTypeScale === 0 || lineTypeScale === 1){
                    ...
                    let ltypeBuffer = [];
                    PromiseEach(ltypes, (value)=>{ return ltypeBuffer.push(value * lineTypeScale)})
                        .then(()=>{
